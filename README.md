...
| `ChirpTone.START_HZ` | `800` (Hz) | `glass/src/main/java/dev/erinlkolp/glassnotify/glass/ChirpTone.java` | **Starting value, tuned on hardware.** Sweep start frequency, selected during the 2026-08-13 spike. |
| `ChirpTone.END_HZ` | `2400` (Hz) | same | **Starting value, tuned on hardware.** Sweep end frequency. Lower this first if the chirp proves audible to bystanders — see [Known limitations](#known-limitations-and-parked-items) — leakage worsens with frequency. |
| `ChirpTone.DURATION_MS` | `150` (ms) | same | **Starting value, tuned on hardware.** Sweep duration; short enough that even an audible leak reads as a click rather than a recognisable alert. |
| `ChirpPlayer.INITIAL_VOLUME_INDEX` | `5` (of 7 max) | `glass/src/main/java/dev/erinlkolp/glassnotify/glass/ChirpPlayer.java` | **Starting value, tuned on hardware.** `STREAM_NOTIFICATION` level written once per install; a deliberate step down from the full-scale tones auditioned during the spike, since the device ships at 7. If it proves audible to bystanders and lowering `ChirpTone.END_HZ` isn't enough, lower this next. |
